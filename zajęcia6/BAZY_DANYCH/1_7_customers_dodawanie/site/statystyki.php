...
echo"<h3>Wzrost - $_SESSION[limit] $sort osoby 
<a href=\"?sortHeight=desc\"><ion-icon name=\"arrow-down-outline\"></ion-icon></a>  <a href=\"?sortHeight=asc\"><ion-icon name=\"arrow-up-outline\"></ion-icon></a></h3>";
...
$sql = "SELECT customers.name,customers.surname,customers.birthday,year(customers.birthday) AS rok FROM customers INNER JOIN cities ON customers.cities_id=cities.id WHERE birthday IS NOT NULL ORDER BY birthday LIMIT $_SESSION[limit]";

$result = $connect->query($sql);

echo "<br>";
echo"<h3>Wzrost - $_SESSION[limit] najstarsze osoby</h3>";
